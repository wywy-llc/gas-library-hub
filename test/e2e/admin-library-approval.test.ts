...
const TEST_SCOPE = `APPROVAL_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
...
test.describe('Admin Screen - Library Approval', () => {
  test('ステータス更新時の確認ダイアログ - キャンセル時は変更されない', async ({ page }) => {
    ...
    const libraryId = await DatabaseLibraryDataFactory.create({
      status: 'pending',
      scriptId: `${TEST_SCOPE}_CANCEL_${Date.now()}`,
      repositoryUrl: `https://github.com/test/${TEST_SCOPE}-cancel-${Date.now()}`,
    });
  ...
  test('ステータス更新中のローディング状態 - ボタンが無効化される', async ({ page }) => {
    ...
    const libraryId = await DatabaseLibraryDataFactory.create({
      status: 'pending',
      scriptId: `${TEST_SCOPE}_LOADING_${Date.now()}`,
      repositoryUrl: `https://github.com/test/${TEST_SCOPE}-loading-${Date.now()}`,
    });
    ...
    // ボタンにcursor-pointerクラスが適用されていることを確認
  ...
  test('複数のライブラリで独立してステータス管理される', async ({ page }) => {
    ...
    // 2つのライブラリを未公開状態でデータベースに直接作成
    const library1Id = await DatabaseLibraryDataFactory.create({
      status: 'pending',
      scriptId: `${TEST_SCOPE}_MULTI1_${timestamp}`,
      repositoryUrl: `https://github.com/test/${TEST_SCOPE}-multi1-${timestamp}`,
    });

    const library2Id = await DatabaseLibraryDataFactory.create({
      status: 'pending',
      scriptId: `${TEST_SCOPE}_MULTI2_${timestamp}`,
      repositoryUrl: `https://github.com/test/${TEST_SCOPE}-multi2-${timestamp}`,
    });
